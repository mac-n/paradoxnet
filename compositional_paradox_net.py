...
class LayerStats:
    ...
    composition_entropy: float = 0.0  # NEW: Track how compositional the patterns are

class ComposltionalDiscretePatternLayer(nn.Module):
    """
    Enhanced DiscretePatternLayer with hierarchical composition.
    
    Key addition: Patterns in layer N are built from layer N-1's patterns.
    Everything else (temporal processing, self-paradox, routing) stays the same.
    """
    ...
    def __init__(self, input_dim, hidden_dim, next_dim, penultimate_dim, n_patterns=8,
        ...
        # HIERARCHICAL PATTERN COMPOSITION - This is the new part!
        if composition_from_prev and prev_layer is not None:
            # Layer N patterns are composed from Layer N-1 patterns
            self.composition_weights = nn.Parameter(
                torch.randn(n_patterns, prev_layer.n_patterns) / (prev_layer.n_patterns ** 0.5)
            )
            self.prev_layer = prev_layer
            # Pattern dict will be computed dynamically from previous layer
        ...
        # Next layer prediction (unchanged)
        ...
        # Pattern attention (needs to work with computed patterns)
    ...
    def get_pattern_dict(self):
        """
        Get the current pattern dictionary - either computed from previous layer or fixed.
        """
        if self.composition_weights is not None and self.prev_layer is not None:
            # Hierarchical composition: patterns = weighted combination of previous patterns
            prev_patterns = self.prev_layer.get_pattern_dict()
            
            # Compose: (n_patterns, prev_n_patterns) @ (prev_n_patterns, hidden_dim) = (n_patterns, hidden_dim)
            composed_patterns = self.composition_weights @ prev_patterns
            
            return composed_patterns
        else:
            # Base layer: fixed patterns
    ...
    def compress_activity(self, x: torch.Tensor, is_next_layer: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        """Compress activity using pattern dictionary (works with composed patterns)"""
        if x.dim() == 1:
            x = x.unsqueeze(0)
        ...
        if is_next_layer:
            ...
            patterns = self.get_pattern_dict()  # Use composed patterns!
    ...
    def apply_self_paradox_nonlinearity(self, x: torch.Tensor) -> torch.Tensor:
        """Apply self-prediction paradox as nonlinearity (unchanged)"""
    ...
    def forward(self, x: torch.Tensor, next_layer: Optional['ComposltionalDiscretePatternLayer'], 
                layer_idx: int) -> Tuple[Optional[torch.Tensor], torch.Tensor, Optional[torch.Tensor]]:
        """Forward pass with hierarchical patterns"""
        if x.dim() == 1:
            x = x.unsqueeze(0)
        ...
        if next_layer is not None:
            # Compress and predict next layer (now uses composed patterns)
            ...
            # Calculate composition entropy for new patterns
            composition_entropy = 0.0
            if self.composition_weights is not None:
                with torch.no_grad():
                    # Measure how diverse the composition weights are
                    comp_weights_norm = F.softmax(self.composition_weights, dim=-1)
                    composition_entropy = -torch.sum(comp_weights_norm * torch.log(comp_weights_norm + 1e-10)).item()
            
            # Track statistics (enhanced with composition info)
...
class ComposltionalDiscretePatternPredictiveNet(nn.Module):
    """
    Enhanced DiscretePatternPredictiveNet with hierarchical pattern composition.
    
    Now layer N's patterns are built from layer N-1's patterns!
    """
    ...
    def __init__(self, input_dim, hidden_dims, penultimate_dim, output_dim, n_patterns=8,
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            ...
            layer = ComposltionalDiscretePatternLayer(
    ...
    def get_layer_stats(self) -> List[LayerStats]:
        """Get statistics including composition entropy"""
        return [layer.last_stats for layer in self.layers if layer.last_stats is not None]
    
    def get_composition_stats(self) -> List[float]:
        """Get composition entropy for all layers (measures hierarchical structure)"""
        stats = self.get_layer_stats()
        return [stat.composition_entropy for stat in stats]
...
# Factory function for your experimental harness
def create_compositional_ppn(sequence_length=20, hidden_dims=[64, 32, 16], n_patterns=8):
    """Create compositional version for comparison with base version"""
    return ComposltionalDiscretePatternPredictiveNet(
...
# Test function to verify composition is working
def test_compositional_structure():
    """Test that hierarchical composition is actually working"""
    
    print("🧪 TESTING COMPOSITIONAL STRUCTURE 🧪")
    
    # Create network
    net = create_compositional_ppn()
    ...
    for i, layer in enumerate(net.layers):
        if layer.composition_weights is not None:
            print(f"Layer {i}: Composes from {layer.composition_weights.shape[1]} patterns in layer {i-1}")
            print(f"  Composition weights shape: {layer.composition_weights.shape}")
            print(f"  Pattern dict shape: {layer.get_pattern_dict().shape}")
        else:
            print(f"Layer {i}: Base layer with {layer.pattern_dict.shape[0]} independent patterns")
    
    # Check composition entropy
    comp_stats = net.get_composition_stats()
    print(f"\nComposition entropies: {comp_stats}")
    print("(Higher = more diverse composition, Lower = more focused)")
    
    return net

if __name__ == "__main__":
    test_compositional_structure()