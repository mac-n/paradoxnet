...
class UltimateExperimentHarness:
    def __init__(self, data_generator: Callable, epochs: int = 50, batch_size: int = 32, 
                 lambda_residual: float = 1.0, save_path: str = 'ultimate_model_tuned.pt'):
        self.data_generator = data_generator
        self.epochs = epochs
        self.batch_size = batch_size
        self.lambda_residual = lambda_residual  # CRANKED UP TO 1.0!
    ...
    def run_trial(self, seed: int):
        ...
        print("🚀 FIXED ULTIMATE ParadoxNet - Residual loss actually works now!")
        print(f"📊 Using TUNED two-way loss: CrossEntropy + {self.lambda_residual} * RecursiveResidualLoss")
        print(f"💪 High residual weight = Strong pressure for interpretability!")
        print("=" * 80)
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch in train_loader:
                ...
                # THE ULTIMATE FORWARD PASS (now with REAL residual loss!)
                final_output, total_prediction_error, recursive_residual_loss = model(X_batch, y_batch)
                
                # Two-way loss: Task performance + HIGH interpretability pressure
            ...
            # Print comprehensive stats (now residual loss should be NON-ZERO!)
...
if __name__ == "__main__":
    print("🔥 FIXED ULTIMATE PARADOX NET - RESIDUAL LOSS TUNED! 🔥")
    print("Bug fixed: Last layer now actually produces residual signal!")
    print("Weight cranked to 1.0 - Maximum interpretability pressure!")
    print()
    ...
    harness = UltimateExperimentHarness(
        data_generator, 
        epochs=50, 
        lambda_residual=1.0,  # MAXIMUM INTERPRETABILITY PRESSURE!
        save_path='ultimate_paradox_model_tuned.pt'
    )
    ...
    print("Let's see if the residual loss is actually working now! 🚀")