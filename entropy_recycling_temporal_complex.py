...
class TemporalEntropyRecyclingLayerComplex(nn.Module):
    """Temporal entropy recycling layer with complex space + RoPE"""
    ...
    def forward(self, x: torch.Tensor, next_layer: Optional['TemporalEntropyRecyclingLayerComplex'], 
        ...
        """Forward pass with complex temporal entropy extraction - SEQUENCE-AWARE"""
        ...
        # Extract patterns and entropy (complex with interleaving)
...
class TemporalEntropyRecyclingNetComplex(nn.Module):
    """Complete temporal entropy recycling network with complex space + RoPE"""
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dims, n_patterns=8):
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            ...
            layer = TemporalEntropyRecyclingLayerComplex(
        ...
        # Final output (real)
        self.final = nn.Linear(hidden_dims[-1] // 2, vocab_size)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        # FULL SEQUENCE PROCESSING (like original ParadoxNet Complex!)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 0:
                ...
                # current_seq is [batch, seq, complex_dim], temporal_entropy is [batch, complex_dim]
        ...
        # Combine penultimate contributions (consensus view) + recursive residual (like original ParadoxNet)
        consensus_view = torch.sum(torch.stack(penultimate_contributions), dim=0)
        penultimate_input = consensus_view + recursive_residual.real  # Add recursive residual like original
        
        # Final output
        output = self.final(penultimate_input)
        
        # Combine errors properly
        if all_errors:
            # Debug: print shapes to understand the issue
            # print(f"Error shapes: {[e.shape for e in all_errors]}")
        ...
        return output, combined_errors, temporal_prediction_error

# Factory function
def create_temporal_entropy_recycling_net_complex(sequence_length=20, hidden_dims=[64, 64, 64], n_patterns=8):
    """Create complex temporal entropy recycling version with RoPE"""
    return TemporalEntropyRecyclingNetComplex(
...
if __name__ == "__main__":
    ...
    # Create network
    net = create_temporal_entropy_recycling_net_complex()
