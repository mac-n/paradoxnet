...
# Import the new, per-pattern temperature model with all mechanisms
from complete_paradox_net_per_pattern_temp import CompleteParadoxNetPerPatternTemp
...
class ParadoxNetText(nn.Module):
    """Wrapper for the new per-pattern temperature model."""
    def __init__(self, sequence_length, vocab_size, embedding_dim=16, hidden_dims=[64, 32], penultimate_dim=32, n_patterns=8, temporal_lr=0.1, temp_lr=0.1):
        ...
        self.ppn = CompleteParadoxNetPerPatternTemp(
            ...
            temp_lr=temp_lr # Pass the adaptive temp learning rate
        )
...
# --- Model Factory ---
def create_per_pattern_temp_net(sequence_length, vocab_size, **kwargs):
    return ParadoxNetText(
        ...
        penultimate_dim=32,
        n_patterns=16,
        temporal_lr=0.5, # Controls stability regularization
        temp_lr=0.1      # Controls adaptive "surprise" regularization
    )
...
class ExperimentHarness:
    ...
    def run_trial(self, seed: int):
        ...
        print("Starting training for the Per-Pattern Temperature Paradox Net...")
...
if __name__ == "__main__":
    ...
    model_factory = create_per_pattern_temp_net
