...
# Note: This version contains the corrected logic for the recursive residual signal.
...
class DiscretePatternLayer(nn.Module):
    """Hidden layer with corrected residual logic."""
    ...
    def forward(self, x: torch.Tensor, next_layer: nn.Module, layer_idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if isinstance(next_layer, PenultimatePatternLayer):
            # CORRECTED LOGIC: This is the last hidden layer. Its job is to produce the final residual.
            # It does not route; it passes its entire state up.
            continue_up = hidden
            # Its contribution to the "consensus view" is zero.
            penultimate_contribution = torch.zeros(x.size(0), self.penultimate_dim, device=x.device)
            # Its prediction error is also zero as it makes no prediction.
            pred_error = torch.zeros(x.size(0), 1, device=x.device)
        else:
            # This is a regular hidden layer, do the prediction and routing as before.
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            self.last_stats.confidence_values = locals().get('confidence', torch.ones_like(pred_error))
...
class PenultimatePatternLayer(nn.Module):
    """Penultimate layer (unchanged from the last version)."""
...
class ParadoxNetResidualLoss(nn.Module):
