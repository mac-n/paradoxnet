...
from entropy_recycling_sequence_simple import SequenceOutputTemporalNet
from data_generators import get_tiny_shakespeare_data

@dataclass
class SequenceTemporalConfig:
    """Configuration for sequence temporal entropy recycling experiment"""
...
def run_sequence_temporal_experiment(config: SequenceTemporalConfig):
    """Run sequence-level temporal entropy recycling experiment"""
    
    print(f"📚 SEQUENCE TEMPORAL ENTROPY RECYCLING EXPERIMENT 📚")
    print(f"Device: {config.device}")
    print(f"Hidden dims: {config.hidden_dims}")
    print(f"Patterns: {config.n_patterns}")
    print(f"Processing: Per-position through temporal entropy recycling")
    print(f"Output: Full sequence predictions")
    ...
    # Create model
    model = SequenceOutputTemporalNet(
    ...
    for epoch in range(config.epochs):
        ...
        for batch_idx, (sequences, targets) in enumerate(train_data):
            ...
            # Get sequence outputs: [batch, seq, vocab_size]
            outputs, errors, temporal_error = model(sequences)
            
            # For language modeling, we predict the NEXT token
            # So we use outputs[:-1] to predict targets (which is sequences[1:])
            # But our current setup has targets as single tokens, so we use the last output
            task_loss = criterion(outputs[:, -1, :], targets)  # Predict last token
        ...
        if epoch % 5 == 0 and epoch_entropy_stats:
            print("  Sequence temporal entropy stats:")
    ...
    print(f"Temporal Entropy (mean pooled): ~3.13")
    print(f"Sequence Temporal Entropy: {best_test_loss:.4f}")
    ...
    print(f"  vs Temporal (mean pooled): {gap_from_temporal:+.1f}%")
    
    if best_test_loss < 3.13:
        print("🎉 Sequence temporal beats mean pooled temporal!")
    ...
    if best_test_loss < 2.65:
        print("🏆 BEATING THE TRANSFORMER BASELINES!")
    ...
    results_file = f"sequence_temporal_entropy_results_{timestamp}.json"
...
if __name__ == "__main__":
    # Run experiment with sequence temporal configuration
    config = SequenceTemporalConfig(
        embedding_dim=64,
    ...
    results = run_sequence_temporal_experiment(config)