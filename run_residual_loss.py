import torch
print("--- EXECUTING THE RESIDUAL LOSS SCRIPT ---")
...
# Import the new, residual loss model
from paradox_net_residual_loss import ParadoxNetResidualLoss
...
class ParadoxNetText(nn.Module):
    """Wrapper for the new residual loss model."""
    def __init__(self, sequence_length, vocab_size, embedding_dim=16, hidden_dims=[64, 32], penultimate_dim=32, n_patterns=8, temporal_lr=0.1, temp_lr=0.1):
        ...
        self.ppn = ParadoxNetResidualLoss(
...
# --- Model Factory ---
def create_residual_loss_net(sequence_length, vocab_size, **kwargs):
...
class ExperimentHarness:
    def __init__(self, data_generator: Callable, model_factory: Callable, epochs: int = 50, batch_size: int = 32, residual_loss_weight: float = 0.1):
        self.data_generator = data_generator
        self.model_factory = model_factory
        self.epochs = epochs
        self.batch_size = batch_size
        self.criterion = nn.CrossEntropyLoss()
        # New hyperparameter for the residual loss weight
    ...
    def run_trial(self, seed: int):
        ...
        print("Starting training for the Recursive Residual Loss Paradox Net...")
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch in train_loader:
                ...
                # KEY CHANGE: The model now returns a third value
                final_output, pred_errors, residual_loss = model(X_batch, y_batch)
                
                task_loss = self.criterion(final_output, y_batch.long())
                
                # The total loss now has three components
                total_loss = task_loss + 0.1 * torch.mean(pred_errors) + self.residual_loss_weight * residual_loss
            ...
            model.eval()
            with torch.no_grad():
                # Note: y_test is still needed for the forward pass
                test_output, _, _ = model(X_test, y_test)
...
if __name__ == "__main__":
    ...
    model_factory = create_residual_loss_net
    
    # We can easily tune the new weight here
    harness = ExperimentHarness(data_generator, model_factory, epochs=50, residual_loss_weight=0.1)
