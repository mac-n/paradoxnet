...
class IntegrationLayerWithPatternMomentum(nn.Module):
    """Integration layer: PERFECT TEMPORAL SYMMETRY with Pattern Evolution Prediction!"""
    ...
    def __init__(self, input_dim, hidden_dim, next_dim, penultimate_dim, output_dim, n_patterns=8):
        ...
        # PATTERN EVOLUTION PREDICTION (TOP LAYER TEMPORAL SYMMETRY!)
        self.pattern_evolution_predictor = ComplexLinear(hidden_dim, n_patterns * (hidden_dim // 2))
        
        # Storage for pattern momentum (like entropy storage in bottom layer)
        self.previous_patterns = None
        self.stored_pattern_evolution_prediction = None
        ...
        self.final_output = nn.Linear(penultimate_dim, output_dim)  # Direct vocab
        
        # Stats
        self.last_stats: Optional[LayerStats] = None
    
    def forward(self, continue_up: torch.Tensor, consensus_contributions: torch.Tensor, 
                targets: Optional[torch.Tensor] = None, layer_idx: int = -1) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Integration layer with pattern evolution prediction: continue_up + consensus → three signals!"""
        ...
        attn = self.pattern_attention(hidden_real_interleaved)
        pattern_weights = F.softmax(attn, dim=-1)
        current_patterns = torch.einsum('bp,pk->bk', pattern_weights.cfloat(), self.pattern_dict)
        
        # SIGNAL 1: Main processing flow → vocab output (PRIMARY)
        ...
        # SIGNAL 2: Pattern evolution prediction loss (TEMPORAL SYMMETRY TOP!)
        pattern_evolution_error = None
        
        # Predict how patterns should evolve
        predicted_evolution = self.pattern_evolution_predictor(hidden)  
        predicted_evolution_reshaped = predicted_evolution.view(-1, self.n_patterns, self.hidden_dim // 2).mean(dim=1)
        
        # If we have previous patterns, compute actual evolution and prediction error
        if self.previous_patterns is not None:
            # Match batch sizes for all tensors
            batch_size = min(current_patterns.size(0), self.previous_patterns.size(0), predicted_evolution_reshaped.size(0))
            current_matched = current_patterns[:batch_size]
            previous_matched = self.previous_patterns[:batch_size]
            pred_evolution_matched = predicted_evolution_reshaped[:batch_size]
            
            # Actual pattern evolution (what really happened)
            actual_evolution = current_matched - previous_matched
            
            # Compare prediction vs reality (both complex) - ensure same batch size
            pred_flat = torch.view_as_real(pred_evolution_matched).flatten(1)
            actual_flat = torch.view_as_real(actual_evolution).flatten(1)
            
            pattern_evolution_error = F.mse_loss(
                pred_flat,
                actual_flat,
                reduction='none'
            ).mean(dim=-1, keepdim=True) * 10000  # Scale up to avoid numerical underflow (less aggressive than 1000000)
            
            # PATTERN MOMENTUM: Accelerate patterns based on prediction accuracy
            with torch.no_grad():
                prediction_accuracy = torch.exp(-pattern_evolution_error)  # Higher accuracy = closer to 1
                momentum_strength = 0.1 * prediction_accuracy
                # Apply momentum to accelerate pattern evolution
                accelerated_patterns = current_matched + momentum_strength.cfloat() * actual_evolution
                # Update pattern dict with momentum (small update)
                self.pattern_dict.data += 0.01 * (accelerated_patterns.mean(0, keepdim=True) - self.pattern_dict.data)
        
        # Store current patterns for next epoch (like entropy storage)
        self.previous_patterns = current_patterns.detach()
        self.stored_pattern_evolution_prediction = predicted_evolution_reshaped.detach()
        
        # Track stats
        self.last_stats = LayerStats(
            prediction_errors=pattern_evolution_error.detach() if pattern_evolution_error is not None else torch.zeros(1, 1),
            ...
            self_paradox_magnitude=torch.mean(torch.norm(hidden.detach(), dim=-1)).item(),
            composition_alpha=0.0,
            entropy_magnitude=torch.mean(torch.norm(hidden - current_patterns, dim=-1)).item(),
            temporal_entropy_magnitude=0.0
        )
        
        return direct_output, penultimate_features, None, pattern_evolution_error

class TemporalSymmetryPatternMomentumNetComplex(nn.Module):
    """Complete temporal symmetry: PAST entropy ↔ FUTURE pattern evolution + MOMENTUM!"""
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dims, n_patterns=8):
        ...
        # INTEGRATION LAYER WITH PATTERN MOMENTUM (FINAL LAYER IN STACK!)
        self.integration_layer = IntegrationLayerWithPatternMomentum(
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Forward pass with TEMPORAL SYMMETRY + PATTERN MOMENTUM: Past entropy ↔ Future patterns"""
        ...
        # INTEGRATION LAYER WITH PATTERN MOMENTUM (PERFECT SYMMETRY!)
        ...
        # INTEGRATION LAYER: continue_up + consensus → output + pattern evolution prediction!
        output, integration_penultimate, _, pattern_evolution_error = self.integration_layer(
        ...
        return output, temporal_prediction_error, pattern_evolution_error

# Factory function
def create_temporal_symmetry_pattern_momentum_net(sequence_length=20, hidden_dims=[64, 64, 64], n_patterns=8):
    """Create temporal symmetry pattern momentum version: PAST ↔ FUTURE + MOMENTUM"""
    return TemporalSymmetryPatternMomentumNetComplex(
...
if __name__ == "__main__":
    print("🚀⚡ TESTING TEMPORAL SYMMETRY + PATTERN MOMENTUM: PAST ↔ FUTURE + ACCELERATION ⚡🚀")
    
    # Create network
    net = create_temporal_symmetry_pattern_momentum_net()
    
    # Test data
    x = torch.randint(0, 57, (5, 10))  # Token sequences
    
    # Forward pass
    output, temporal_error, pattern_evolution_error = net(x)
    
    print(f"Input: {x.shape}, Output: {output.shape}")
    print(f"Temporal error: {temporal_error.item() if temporal_error is not None else None}")
    print(f"Pattern evolution error: {pattern_evolution_error.item() if pattern_evolution_error is not None else None}")
    
    # Check temporal symmetry statistics
    print(f"\n=== TEMPORAL SYMMETRY + PATTERN MOMENTUM STATISTICS ===")
    for i, layer in enumerate(net.layers):
        if layer.last_stats:
            stats = layer.last_stats
            print(f"Layer {i} ({'BOTTOM' if layer.is_bottom else 'MIDDLE'}):") 
    ...
    print(f"\n✅ TEMPORAL SYMMETRY + PATTERN MOMENTUM: Past entropy ↔ Future pattern evolution + Acceleration working!")
    print(f"🎯 Perfect three-loss structure: task + temporal + pattern_evolution")