...
class SequenceTemporalEntropyRecyclingLayer(nn.Module):
    """Temporal entropy recycling layer with sequence-level processing"""
    ...
    def __init__(self, input_dim, hidden_dim, next_dim, penultimate_dim, n_patterns=8,
        ...
        # SEQUENCE-LEVEL TEMPORAL ENTROPY PROCESSING (special for bottom layer)
        if is_bottom:
            ...
            # Sequence aggregation for temporal entropy
            self.temporal_sequence_aggregator = nn.Linear(hidden_dim, hidden_dim)
    ...
    def extract_patterns_and_entropy(self, hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Extract patterns and compute entropy as residual - sequence aware"""
        patterns = self.get_pattern_dict()
        
        # Attention over patterns (works with sequences)
        attn = self.pattern_attention(hidden)  # [batch, seq, n_patterns]
        pattern_weights = F.softmax(attn, dim=-1)
        
        # Pattern reconstruction (what patterns can explain)
        # Handle variable dimensions - flatten extra dims for matmul
        original_shape = pattern_weights.shape
        if pattern_weights.dim() > 3:
            # Flatten extra dimensions: [batch, ..., n_patterns] -> [batch*..., n_patterns]
            pattern_weights_flat = pattern_weights.view(-1, pattern_weights.size(-1))
            pattern_reconstruction_flat = pattern_weights_flat @ patterns
            # Reshape back: [batch*..., features] -> [batch, ..., features]
            pattern_reconstruction = pattern_reconstruction_flat.view(*original_shape[:-1], patterns.size(-1))
        elif pattern_weights.dim() == 3:
            # Standard sequence case: [batch, seq, n_patterns] @ [n_patterns, features]
            pattern_reconstruction = torch.einsum('bsp,pf->bsf', pattern_weights, patterns)
        else:
            # Simple case
    ...
    def apply_self_paradox_nonlinearity(self, x: torch.Tensor, temporal_entropy: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Sequence-aware paradox mechanism with temporal entropy integration"""
        # Process input (sequence-aware)
        hidden_linear = self.process(x)  # [batch, seq, hidden_dim]
        
        # Temporal entropy integration for bottom layer
        if self.is_bottom and temporal_entropy is not None:
            # Handle sequence dimension for temporal entropy
            batch_size, seq_len = hidden_linear.shape[:2]
            
            # Temporal entropy might be [batch, hidden_dim] from previous epoch
            if temporal_entropy.dim() == 2:
                # Expand to sequence: [batch, hidden_dim] -> [batch, seq, hidden_dim]
                temporal_entropy = temporal_entropy.unsqueeze(1).expand(-1, seq_len, -1)
            ...
            if temp_batch_size != batch_size:
                if temp_batch_size < batch_size:
                    ...
                    temporal_entropy = temporal_entropy.repeat(repeats, 1, 1)[:batch_size]
                else:
        ...
        # Self-prediction paradox mechanism (sequence-aware)
    ...
    def forward(self, x: torch.Tensor, next_layer: Optional['SequenceTemporalEntropyRecyclingLayer'], 
        ...
        """Forward pass with sequence-level temporal entropy extraction"""
        ...
        # Extract patterns and entropy (sequence-aware)
        ...
        if next_layer is not None:
            # Predict next layer (sequence-aware)
            ...
            # Prediction error (average over sequence)
            pred_error = torch.mean((actual_patterns - predicted_next)**2, dim=[1, 2], keepdim=True)
            ...
            # Route information (sequence-aware)
            penultimate_features = self.to_penultimate(pattern_reconstruction)
            
            # Always average to [batch, features] for penultimate
            while penultimate_features.dim() > 2:
                penultimate_features = penultimate_features.mean(dim=1)
            penultimate_contribution = penultimate_features * confidence
                
            # Handle confidence broadcasting for continue_up
            confidence_broadcast = confidence
            for _ in range(hidden.dim() - confidence.dim()):
                confidence_broadcast = confidence_broadcast.unsqueeze(-1)
            continue_up = hidden * (1 - confidence_broadcast)
            ...
            self.last_stats = LayerStats(
                ...
                continue_magnitude=torch.mean(torch.norm(continue_up.detach(), dim=[1, 2])),
                layer_idx=layer_idx,
                pattern_usage=pattern_weights.detach().mean([0, 1]),  # Average over batch and sequence
                pattern_entropy=0.0,
                self_paradox_magnitude=torch.mean(torch.norm(hidden.detach(), dim=[1, 2])).item(),
                composition_alpha=composition_alpha,
                entropy_magnitude=torch.mean(torch.norm(entropy, dim=[1, 2])).item(),
            ...
            # Last layer processing (sequence-aware)
            penultimate_features = self.to_penultimate(pattern_reconstruction)
            # Always average to [batch, features] for penultimate
            while penultimate_features.dim() > 2:
                penultimate_features = penultimate_features.mean(dim=1)
            penultimate_contribution = penultimate_features
            ...
            self.last_stats = LayerStats(
                ...
                layer_idx=layer_idx,
                pattern_usage=pattern_weights.detach().mean([0, 1]),
                pattern_entropy=0.0,
                self_paradox_magnitude=torch.mean(torch.norm(hidden.detach(), dim=[1, 2])).item(),
                composition_alpha=composition_alpha,
                entropy_magnitude=torch.mean(torch.norm(entropy, dim=[1, 2])).item(),
...
class SequenceTemporalEntropyRecyclingNet(nn.Module):
    """Complete sequence-level temporal entropy recycling network"""
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dims, n_patterns=8):
        ...
        # Create sequence-level temporal entropy recycling layers
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            ...
            layer = SequenceTemporalEntropyRecyclingLayer(
        ...
        # Final output (sequence-to-sequence)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Forward pass with sequence-level temporal entropy recycling"""
        # Embedding + positional encoding
        embedded = self.embedding(x)  # [batch, seq, embed_dim]
        embedded = self.pos_encoder(embedded)  # Add positional encoding
        
        # Process sequences through layers
        penultimate_contributions = []
        current = embedded  # Keep full sequence: [batch, seq, embed_dim]
        all_errors = []
        all_entropy = []
        ...
        if all_entropy:
            # Average entropy over sequence for next epoch
            total_entropy = torch.stack([e.mean(dim=1) for e in all_entropy]).sum(dim=0)
        ...
        # Final output - predict next token for each position
        output = self.final(penultimate)
        
        return output, torch.cat(all_errors, dim=1) if all_errors else None, temporal_prediction_error

# Factory function
def create_sequence_temporal_entropy_recycling_net(sequence_length=20, hidden_dims=[64, 64, 64], n_patterns=8):
    """Create sequence-level temporal entropy recycling version"""
    return SequenceTemporalEntropyRecyclingNet(
...
if __name__ == "__main__":
    print("📚 TESTING SEQUENCE TEMPORAL ENTROPY RECYCLING 📚")
    
    # Create network
    net = create_sequence_temporal_entropy_recycling_net()
    ...
    print(f"\n=== SEQUENCE TEMPORAL ENTROPY STATISTICS ===")
    ...
    print(f"\n✅ Sequence temporal entropy recycling working!")