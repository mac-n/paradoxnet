...
# Note: This version RE-INTRODUCES the adaptive (self-prediction) temperature
# and combines it with the per-pattern temporal temperature.
...
class PenultimatePatternLayer(nn.Module):
    """Penultimate layer with combined temperature mechanisms."""
...
class CompleteParadoxNetPerPatternTemp(nn.Module):
    def __init__(self, input_dim, hidden_dims, penultimate_dim, output_dim, n_patterns=8, temporal_lr=0.1, temp_lr=0.1):
        ...
        self.penultimate_layer = PenultimatePatternLayer(
            penultimate_dim, penultimate_dim, output_dim, n_patterns, temporal_lr, temp_lr
        )
